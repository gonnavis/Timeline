...
.controller('timelineCtrl',function($scope,Timeline,view,$element){
	...
	//fn
		$s.panTo=function(year,bias){
			// debugger
			var to=(-$s.min+year)*$s.zoom+bias;
			jq('body').scrollLeft(to);
		}
		window.panTo=$s.panTo;
		$s.wheel=function(e,d,x,y){
			if(e.originalEvent.ctrlKey||e.originalEvent.altKey||e.originalEvent.shiftKey){
				return;
			}
			if(d>0){
				$s.zoomIn();
				$s.panTo($s.mouseTime,-e.originalEvent.clientX);
			}
			else{
				$s.zoomOut();
				$s.panTo($s.mouseTime,-e.originalEvent.clientX);
